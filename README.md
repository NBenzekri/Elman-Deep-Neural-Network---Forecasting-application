# Elman-Deep-Neural-Network---Forecasting-application
Elman Deep Neural Network in Forecasting application using R programming tested on 3 types of dataset
## A10 dataset results:
#### A10 QQplot
![qqplot](https://i.imgur.com/WU1uDtA.png)
#### Results
![A10 dataset](https://i.imgur.com/nSseic4.png)
## Cafe dataset results:
![Cafe dataset](https://i.imgur.com/qecBMiE.png)
## Sun Spot dataset results:
#### With Elman Neural net
![Elman Model](https://i.imgur.com/bARI87T.png)
#### With ARIMA model:
![Arima model ](https://i.imgur.com/Jd2sm9T.png)


